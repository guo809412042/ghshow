...
import { AdvCompanyList, advTypeList } from './utils';
    ...
    return (<Fragment>
      ...
      <Modal title="添加广告位" width={500} visible={this.state.visible} onCancel={() => this.setState({ visible: false })} onOk={this.submit}>
        <Form ref={this.formRef} layout="horizontal">
          ...
          <FormItem {...formItemLayout} label="广告形式">
            {getFieldDecorator('disptype', {
              initialValue: content.disptype,
              rules: [{ required: true }],
            })(
              <Select style={{ width: '200px' }}>
                {
                  advTypeList.map(item => <Option value={item.select_key} key={item.select_key}>{item.select_value}</Option>)
                }
              </Select>,
            )}
          </FormItem>
