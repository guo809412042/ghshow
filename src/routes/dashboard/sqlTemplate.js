...
// android - 次日留存
export const stayRollRegSQL = `
select  sum(active_num) as reg_num
        ,sum(stay_num) as stay_num
        ,ds
from    vcm_vmix_log_dvc_stay_cnt_1d
where   ds >= #startDate#
and     ds <= #endDate#
and     platform = #product#
and     product_id = 18
#quersql#
and new_user in #selectValue#
group by ds
order by ds asc
limit   1000
;
...
export const tempCumulativeUsers = 'select ds, sum(duid_total) AS total from vcm_tempo_total_user where ds >= \'#startDate#\' AND ds <= \'#endDate#\' group by ds order by ds;';
