 ...
 * @LastEditTime: 2021-04-26 15:45:21
...
import {
  checkStateMap, auditStateMap, auditColorMap, auditStatusMap, auditDescMap,
} from './const';
...
export default ({ match: { params } }) => {
  ...
  const columnsInfo = [
    ...
    },
    {
      ...
      render: (text, row) => (selectData.audit_state !== 2 ? format(evaluate(Number(row.deviation_count) + Number(text)), { notation: 'fixed', precision: 2 }) : format(text, { notation: 'fixed', precision: 2 })),
    },
    {
      title: '累计偏差率(%)',
      dataIndex: 'all_deviation_count_rate',
      key: 'all_deviation_count_rate',
      render: (_text, row) => (selectData.audit_state !== 2 ? (Number(row.sumSpendCount) + Number(row.spend_count) ? format(evaluate((Number(row.deviation_count) + +row.sumDeviationCount) / (+row.sumSpendCount + +row.spend_count) * 100), { notation: 'fixed', precision: 2 }) : 0) : (Number(row.sumSpendCount) ? format(evaluate((row.sumDeviationCount) / (row.sumSpendCount) * 100), { notation: 'fixed', precision: 2 }) : 0)),
    },
  ];
  ...
  const initType = async () => {
    if (params.uuid) {
      ...
      if (data.data.feishu_instance_code) {
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginTop: 16 }}>
        <Col span={24}>
          <Steps direction="vertical" current={timeline.current} status={auditStatusMap[timeline.status]}>
            <Steps.Step title="已提交" description={moment(timeline.start_time).format('YYYY-MM-DD HH:mm:ss')} />
            <Steps.Step
              title="审核人"
              description={
                  <>
                    {timeline.task_list && timeline.task_list.map(item => <Steps.Step
                      title=""
                      description={
                          <>
                            <p>时间：{moment(item.start_time).format('YYYY-MM-DD HH:mm:ss')}</p>
                            <p>审核人员: <span style={{ color: '#000' }}>{item.email}</span></p>
                            <p>审核状态：<span style={{ color: auditColorMap[item.status] }}>{auditStateMap[item.status]}</span></p>
                          </>
                      }
                    />)}
            ...
            />
            <Steps.Step title={auditDescMap[timeline.status]} description={timeline.end_time ? moment(timeline.end_time).format('YYYY-MM-DD HH:mm:ss') : '未完成'} />
          </Steps>
        </Col>
      </Row>
      <Row gutter={16} style={{ marginTop: 16 }}>
