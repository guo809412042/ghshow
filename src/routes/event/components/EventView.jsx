...
import {
  Select, Radio, Table, Input, message,
} from 'antd';
...
import { getEventNameList, updateName } from '../services/index';
...
const EventView = ({
  ...
  // 修改事件名称
  const updateEventName = record => async (e) => {
    e.persist();
    setTimeout(async () => {
      if (e.target.value != record.event_name_zh) {
        const params = { event_name: record.event_name, event_name_zh: e.target.value };
        const res = await updateName(params);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          <Input style={{ border: 'none' }} defaultValue={text} onBlur={updateEventName(record)}/>
        </div>
      ),
    },
    {
  ...
  const getFetchData = async () => {
    ...
    const dataSource = res.map(v => ({
      ...v,
      event_name_zh: EventNamesJson[v.event_name] || '',
    }));
